...
function renderStageBlock(evalResult) {
  ...
  let why;
  if (evalResult.stage === 0) {
    const gate1 = evalResult.gates.find((g) => g.stage === 1);
    why = gate1 ? gate1.unmet : '';
  } else {
    const gate = evalResult.gates.find((g) => g.stage === evalResult.stage);
    why = gate ? gate.condition : '';
  }
  return `
    <section class="today-stage">
      <p class="stage-kicker">Current stage</p>
      <h2 class="stage-heading">Stage ${evalResult.stage} — ${escapeHtml(name)}</h2>
      <p class="stage-why">${escapeHtml(why)}</p>
    </section>`;
}
